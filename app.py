...
joblib.dump(preprocessor, "preprocessor.joblib") #dump and custom name

# web application
st.set_page_config(
	page_title="Flights Prices Prediction",
	page_icon="✈️",
	layout="wide" #use entire width
)

st.title("Flights Prices Prediction - AWS SageMaker")

# user inputs #all input columns except price
...
#pd to_datetime function we're using in transformations doesn't work on st's datetime objects.
#convert these dtypes to string so pandas can later convert str to datetime.

if st.button("Predict"): #user clicks predict
    ...
    #the .info is shown in a blue box.